...
class FollowerDialog(context: Context, userName : String) : BottomSheetDialog(context) {


    private val url = "https://api.github.com/users/$userName/followers?per_page=100"

    var followersList = mutableListOf<Follower>()
    var layoutManager: RecyclerView.LayoutManager? = LinearLayoutManager(this.context)
    var adapter: RecyclerView.Adapter<FollowersAdapter.ViewHolder>? =
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.bottom_top)!!.text = Html.fromHtml(
            getColorStr("Your ", "#6CFF54") + getColorStr(
                "Followers", this.context.getColor(
                    R.color.text_color
                ).toString()
            )
        )
